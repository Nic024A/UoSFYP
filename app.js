  ...
  function getAllFromDatabase() {
    ...
    if (unCompletedAssignment.length > 0) {
        unCompletedAssignment.map((item) => {
            ...
            <div class="assignment-date" > Due Date: ${item.duedate}</div>
            <div class="assignment-details" > Notes: ${item.notes}</div>

            </div>
            <div class = "view_assets" onclick="document.getElementById('modal3').style.display='block'; setIdToLocalStorageAsset(${item.id}); " style="width:auto;" >View Assets</div> 
        ...
        //Countdown Banner
        var userDate =  new Date("2019-02-29");
